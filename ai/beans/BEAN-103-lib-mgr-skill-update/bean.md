# BEAN-103: Library Manager — Skill Update
...
In the Library Manager, users cannot edit and save skills. The existing code in library_manager.py has update logic for skills, but it does not work correctly end-to-end — items may not appear in the tree, operations may not persist, or the UI may not refresh properly.

## Goal

A user can select an existing skill, edit its markdown content in the editor, save changes, and confirm the file on disk is updated.

## Scope

### In Scope
- Selecting a skill loads its current content into the editor
...
Depends on BEAN-102 (Skill Create) — need items to exist to test editing.
