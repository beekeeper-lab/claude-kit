...
2. **Process completed workers** — For each worker showing `status: done` that hasn't been merged: remove the worktree, sync (`git fetch origin && git pull origin test`), run `/internal:merge-bean NNN` (default target is `test`), update `_index.md` to `Done` on `test`, commit and push.
